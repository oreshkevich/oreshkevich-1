...
const checkModalElements = (isEdit = false, isRateModal = false) => {
    ...
    cy.viewport(400, 600);
    cy.get('@outer').screenshot(
        isRateModal ? '16-rate-modal-mobile' : isEdit ? 'edit-mobile-view' : '3-mobile-view',
    );
    cy.get('@close').click();
...
describe('booking and book rate', () => {
    ...
    describe('booking modal', () => {
        ...
        it('check calendar header', () => {
            ...
            cy.get('@modal')
                .find('[data-test-id=calendar]')
                .as('calendar')
                .screenshot('4-january', { padding: 10 });
